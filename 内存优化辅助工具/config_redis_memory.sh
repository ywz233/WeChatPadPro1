#!/bin/bash
# Redis内存配置脚本
# 使用方法: ./config_redis_memory.sh [host] [port] [password] [maxmemory] [policy]
...
# 设置最大内存
echo -n "配置最大内存限制... "
$REDIS_CMD CONFIG SET maxmemory $REDIS_MAXMEM > /dev/null 2>&1
if [ $? -ne 0 ]; then
    echo "失败！"
...
# 设置内存策略
echo -n "配置内存淘汰策略... "
$REDIS_CMD CONFIG SET maxmemory-policy $REDIS_POLICY > /dev/null 2>&1
if [ $? -ne 0 ]; then
    echo "失败！"
...
# 持久化配置到redis.conf
echo -n "尝试持久化配置到Redis配置文件... "
$REDIS_CMD CONFIG REWRITE > /dev/null 2>&1
if [ $? -ne 0 ]; then
    echo "失败！"
    echo "无法持久化配置。Redis配置可能是只读的或CONFIG REWRITE不被支持。"
    echo "请手动编辑redis.conf文件，添加或修改以下配置行："
...
# 验证设置
echo -n "验证配置... "
CURRENT_MAXMEM=$($REDIS_CMD CONFIG GET maxmemory | grep -v maxmemory)
CURRENT_POLICY=$($REDIS_CMD CONFIG GET maxmemory-policy | grep -v maxmemory-policy)
